...
| API-04 | API | AC-04, AC-05 | Active IT Staff/Administrator owner options plus queue search, combined filters, sort, pages, assigned/unassigned/mine. | Only active permitted owner summaries are returned; queue data/meta and invalid query `400`. | `server/tests/lab-03/staff-queue.api.test.ts`, `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Pass |
