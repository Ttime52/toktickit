...
| FR-06 | An Administrator MUST search/filter Users, create one User with one role and initial password, edit name/email/role/activation, and reset an initial password. The API MUST prevent duplicate email, self-deactivation and removal of the last active Administrator. |
...
| AC-07 | Given an Administrator, when they manage Users, then create/search/filter/edit/reset work and duplicate email, invalid role, self-deactivation and last-admin removal are blocked. |
